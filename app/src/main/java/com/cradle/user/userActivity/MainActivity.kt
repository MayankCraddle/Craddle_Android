...
import com.cradle.databinding.ActivityMainBinding
import com.cradle.repository.ExceptionHandler
import com.cradle.user.adapters.MoviesAdapter
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
     ...
     /*   mainViewModel.liveDataCounty.observe(this){
            when(it){
        ...
        mainViewModel.liveDataStateList.observe(this){
            when(it){
                is ExceptionHandler.Loading<*> ->{}
                is ExceptionHandler.Success<*> ->{
                    it.data?.let {
        ...
        mainViewModel.liveDataCitiesList.observe(this){
            when(it){
                is ExceptionHandler.Loading<*> ->{}
                is ExceptionHandler.Success<*> ->{
                    it.data?.let {
